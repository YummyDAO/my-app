...
const CONTRACT_ADDRESS2 = '0x9915089F9aA803c9009d62f8f05bCeA7E7e69484'; //stability Pool
const contractAbi2 = [
    {
      "inputs": [
        {
...
const [Withdraw1, setWithdraw1] = React.useState('');
const [Approval, setApproval] = React.useState('');
const [Colgain, setColgain] = React.useState('');


const change1 = (event) => { //deposit
    setDeposit1(event.target.value);
    console.log("Deposit1", Deposit1)
  };

  const change = (event) => { //deposit
    setWithdraw1(event.target.value);
...
const handleChange1 = (event, newValue) => {
  ...
  //connect wallet
    ...
    const { data1, error1 } = useSWR('ReadTotal1', ReadTotal1)
    ...
    const ReadTVL = async () => {
        try {
            ...
            if (ethereum) {
                ...
                let tx = await contract.getTotalDebtTokenDeposits();
                //await tx.wait();
                console.log("white", ethers.formatUnits(tx ? tx.toString(): "0", 18))
                setTVL(ethers.formatUnits(tx ? tx.toString(): "0", 18))
                //return tx
                //console.log("tx", tx[1])
                //setDeposit(tx[1])
            }
    ...
    const { data5, error5 } = useSWR('ReadTVL', ReadTVL)

    console.log("few", ethers.parseUnits(Deposit1 ? Deposit1.toString() : "0", 18))
    ...
    const Deposit = async () => {
        try {
          ...
          if (ethereum) {
              ...
              let tx = await contract.provideToSP(ethers.parseUnits(Deposit1 ? Deposit1.toString() : "0", 18));
              await tx.wait();
    ...
    const Withdraw = async () => {
        try {
          ...
          if (ethereum) {
              ...
              let tx = await contract.withdrawFromSP(ethers.parseUnits(Withdraw1 ? Withdraw1.toString() : "0", 18));
              await tx.wait();
              console.log("approvetx", tx)
          }
        } catch(error) {
    ...
    const Approve = async () => {
        try {
          ...
          if (ethereum) {
              ...
              let tx = await contract.approve(CONTRACT_ADDRESS2, ethers.parseUnits(Deposit1 ? Deposit1.toString() : "0", 18));
              await tx.wait();
              console.log("approvetx", tx)
          }
        } catch(error) {
    ...
    const ClaimEth = async () => {
        try {
          ...
          if (ethereum) {
              ...
              let tx = await contract.claimCollateralGains(currentAccount, [0]);
              await tx.wait();
              console.log("approvetx", tx)
          }
        } catch(error) {
    ...
    const ReadCollgain = async () => {
      try {
        ...
        if (ethereum) {
            ...
            let tx = await contract.getDepositorCollateralGain(currentAccount);
            //await tx.wait();
            console.log("tx", tx[0])
            console.log("ReadCollgain", ethers.formatUnits(tx[0], 18))
            setColgain(ethers.formatUnits(tx[0], 18))
        }
      } catch(error) {
        console.log(error);
      }
    ...
    const { data8, error8} = useSWR('ReadCollgain', ReadCollgain, {refreshInterval: 1000})
    const { data6, error6} = useSWR('ReadBalance', ReadBalance, {refreshInterval: 1000})
    const { data7, error7} = useSWR('ReadTVL', ReadTVL, {refreshInterval: 1000}) //
      ...
      <Main open={open} className='css-142ldf2'>
        <Container maxWidth="lg" className='css-18bd6du'>
            <div className='css-v8rda8'>
                ...
                <Link to="/earn"><Button variant="contained" className='css-1te0lnf'>
            ...
            <Grid container className='css-1ddgjd7'>
                        ...
                        <Card className='css-93jgv3'>
                            ...
                            <CardContent className='css-ha3yzm '>
                                <Grid container className='css-1d3bbye'>
                                    ...
                                    <Grid item xs={6} sm={6} md={6} lg={6} className='css-1s50f5r'>
                                        <Typography className='css-127u7vo'>{Number(TVL).toFixed(2)} igUSD</Typography>
                                    </Grid>
                                </Grid>
                ...
                <Grid item xs={12} sm={12} md={12} lg={12} className='css-1dvm79k'>
                <Card className='css-wcst0s'>
                    <CardHeader title={<Typography className='css-1e2y9ep'>Manage Stability Pool</Typography>} className='css-p9m3fa'></CardHeader>
                    <Typography className='css-1mvwzi1'>Deposit igUSD to earn IGURU rewards. During liquidations, your deposit will be used to purchase discounted collaterals. </Typography>
                    <Box className='css-8atqhb'>
                        <Tabs value={value} onChange={handleChange1} aria-label="basic tabs example" className='css-xlp6dr main-tabs'>
                            <Tab label="Deposit" {...a11yProps(0)}  className='css-r86tzm'/>
                            <Tab label="Withdraw" {...a11yProps(1)} className='css-r86tzm'/>
                        </Tabs>
                                ...
                                <div className='css-1d1ry76'>
                                    <div className='css-gmkiuq'>
                                        <div className='css-1fxv11n'>
                                            <Typography className='css-1irks2t'>Deposit igUSD</Typography>
                                    ...
                                    <div className='mgt10'>
                                        <Button variant="contained" className='css-1r3fpwf' onClick={Deposit}>Deposit</Button>
                                    </div>
                                    { Approval < 1 ?
                                    <div className='mgt10'>
                                    ...
                                    :
                                    <div className='mgt10'>
                                        <Button variant="contained" className='css-1r3fpwf' onClick={ClaimEth}>Claim Eth Rewards {Number(Colgain).toFixed(2)} rETH</Button>
                                    </div>}
                                </div>
                            </CardContent>
                        </CustomTabPanel>
                            ...
                            <CardContent className='css-1q1z20c'>
                                    <div className='css-1d1ry76'>
                                        <div className='css-gmkiuq'>
                                            ...
                                            <div className='css-lffzzs'>
                                                <TextField className='css-skinv8' label="Enter an amount" onChange={change}/>
                                            </div>
                                        </div>
                                        <div className='mgt10'>
                                        <Button variant="contained" className='css-1r3fpwf' onClick={Withdraw}>Withdraw</Button>
                                    </div>
                                    </div>
                                </CardContent>
                        </CustomTabPanel>
